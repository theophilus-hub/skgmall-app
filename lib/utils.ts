const init: Map<string, string[]> = new Map(Object.entries(JSON.parse(`{
...
export const states: SelectItem[] = Array.from(init.keys()).map((state)=>{
    return { label: state, value: state };
});

export const locations = (state: string): SelectItem[] => {
    const results = init.get(state);
    if(results){
